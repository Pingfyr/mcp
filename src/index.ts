...
async function startStdio(): Promise<void> {
  ...
  if (!apiKey || !apiKey.startsWith("rm_")) {
    console.error(
      "Error: PINGFYR_API_KEY environment variable is required and must start with rm_"
    );
    process.exit(1);
  }
  ...
  if (!BASE_URL.startsWith("https://") && !process.env.PINGFYR_ALLOW_HTTP) {
    console.error(
      "Error: PINGFYR_API_URL must use HTTPS. Set PINGFYR_ALLOW_HTTP=1 for development."
    );
    process.exit(1);
  }
